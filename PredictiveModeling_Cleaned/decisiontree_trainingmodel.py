...
for x in labels:
  ...
  pipeline = Pipeline(stages=[stringIndexerTrain, encoder, labelIndexer, assembler, dt, labelConverter])
...
for x in labels:
  ...
  pipeline = Pipeline(stages=[stringIndexerValid, encoder, labelIndexer, assembler, dt, labelConverter])
...
#decisiontree accuracy for all the labels
print accuracylabel
...
#trainoutput.repartition(1).write.format("com.databricks.spark.csv").option("header","true").save("mnt/allstate-gjs/train_output_dt.csv")
